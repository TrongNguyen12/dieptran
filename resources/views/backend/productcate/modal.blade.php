...
$cate=DB::table("product_categories")->select('name','id')->where('display',1)->where('id',$key)->first();
@$catechild=DB::table("product_categories")->select('name','id')->where('display',1)->where('parent_id',$cate->id)->get();
?>

 <div class="modal-header">
    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
      <span aria-hidden="true">×</span></button>
    <h4 class="modal-title"> Danh mục cha: <span style="color: red;">{{$cate->name}}</span></h4>
  </div>
  <div class="modal-body">
                
    	<form method="post" action="backend/productcate/postAddchild"  enctype="multipart/form-data">
           ...
           <h4 class="modal-title"> Danh mục con</h4>
            ...
            <?php foreach(@$catechild as $rows){?> 
                <div class="row" style="margin-bottom: 10px;padding: 10px 0px;border: solid 1px #ddd;">
                  ...
                  <div class="col-xs-4"> <a  href="backend/productcate/{{$rows->id}}/delete">Xóa</a> </div>
               </div>   
            <?php } ?>
            </div>
           </div> 
