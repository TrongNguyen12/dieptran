...
<section class="content">
    ...
    <form style="float: right;margin: 10px 15px 15px 0px;" action="{!! url('backend/product/search') !!}" class="form-group form-inline admin_frm">

        <input name="name" placeholder="Tìm kiếm theo tên" required> 

        <button type="submit" class="admin_search-btn btn_edit">Tìm Kiếm</button>
          ...
          <table id="example2" class="table table-bordered table-hover">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td><a  href="<?php if($item->alias != null){?>{{ asset(@$catpro->alias.'/'.$item->alias.'.'.$item->id.'.html') }} <?php } else { ?>{{ asset(@$catpro->alias.'/'.changeTitle($item->name).'.'.$item->id.'.html') }} <?php } ?>" title="{{ $item->name }}" >{{$item->name}}</a></td>
                ...
                <td>
                    ...
                    <?php if($item->typephoto == 0){?>

                    <img src="{{ asset($item->photo) }}" onerror="this.src='{{asset('public/admin_assets/images/no-image.jpg')}}'" style="max-width: 100%;max-height: 200px;"  alt="NO PHOTO" />
               ...
               <td class="text-center with_dieuhuong">
                  ...
                  <div class="form-group" > 
                    ...
                    @if($item->noibat>0)
                      ...
                      <a href="backend/product/edit?id={{$item->id}}&noibat={{ time() }}" class="btn btn-primary btn-xs"><i class="fa fa-eye"></i> Nổi bật(slide)</a>

                    @else

                      <a href="backend/product/edit?id={{$item->id}}&noibat={{ time() }}" class="btn btn-danger btn-xs"><i class="fa fa-eye"></i> No Nổi bật(slide)</a>
