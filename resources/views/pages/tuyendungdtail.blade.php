...
@section('content' )
	<section class="text-center position-relative sbanner">
		...
		<h1 class="bold s27 text-center sbanner-tit wow fadeInDown" data-wow-delay="300" data-wow-duration=".8s">
			<a href="{{ asset('tin-tuc.html') }}"><span>{{ trans('home.title8') }}</span></a>
		</h1>
	</section>
	<section class="serpage bdetail">
		<div class="container-140">
						...
						<div class="col-sm-6">
							<figcaption class="s14 blog-info">
								<h3 class="bold blog-info-tit">
									<a href="{{ asset('tuyen-dung/'.$item->slug.'-p'.$item->id) }}.html" title="">
										@if (App::isLocale('vn'))
											{{ $item->title }}
										@else
											{{ $item->titleeg }}
			...
			<div class="b4 sdetail-content-wrap">
				<span class="s26 cate text-white"><a href="{{ asset('tuyen-dung')}}.html" title="">{{ $data->Catepost->name }}</a></span>
				...
				<ul class="list-unstyled bdetail-list">
					...
					@foreach ($listPost as $item)
						<li><a href="{{ asset('tuyen-dung/'.$item->slug.'-p'.$item->id) }}.html" title="">
							@if (App::isLocale('vn'))
								{{ $item->title }}
							@else
								{{ $item->titleeg }}
